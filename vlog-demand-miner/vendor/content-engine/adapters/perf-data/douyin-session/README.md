...
被 `/retro` 在 `state.data_collection=adapter` + `platform=douyin` 时自动调用。
...
调试产物（URL dump / 截图）写到 `.nexttake-cache/douyin-session-debug/`，避免散落在 skill 源码目录。
...
# ADAPTER="$NEXTTAKE_CONTENT_ENGINE_ROOT/adapters/perf-data/douyin-session"
...
- `https://v.douyin.com/abc123` → 短链需要 resolve（publish 自动做）

publish 会在登记发布时把 aweme_id 存到 prediction header（如能 resolve）。retro 启动时直接读这个字段。
...
- `.nexttake-cache/douyin-session-debug/` 也不应提交到 git：里面可能含调试截图和接口 URL dump
