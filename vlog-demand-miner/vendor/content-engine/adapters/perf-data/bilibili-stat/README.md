...
回收**自己（或任意）B站视频**的播放数据 + 热门评论，供 `/retro` 复盘。

被 `/retro` 调用：当 `state.data_collection=adapter` 且 `Platform: bilibili` 时。
...
`0` 成功 · `2` 缺依赖（httpx 未装）· `3` 其他失败（网络 / 解析 / BV 号错误）。任何失败时 `/retro` 会优雅降级到 manual 模式。
