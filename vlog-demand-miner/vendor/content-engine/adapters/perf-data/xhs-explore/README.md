...
被 `/retro` 在 `state.data_collection=adapter` + `platform=xhs` 时自动调用。
...
调试产物（URL dump / 截图 / galaxy 原始 JSON）写到 `.nexttake-cache/xhs-explore-debug/`。
...
# 4. 首次扫码登录小红书创作者中心
ADAPTER=$(find ~/NextTake Content Engine -name "xhs-explore" -type d 2>/dev/null | head -1)
python "$ADAPTER/crawler.py" login
# → 弹出 Chromium 窗口，用小红书 App 扫码
# → 登录成功后窗口自动关闭，cookie 存在 当前目录/.auth-xhs/
```

> Windows 提示：adapter 在 `<nexttake-root>/vlog-demand-miner/vendor/content-engine/adapters/perf-data/xhs-explore`，
...
- `https://xhslink.com/xxxxx` → 短链，publish 会 resolve

publish 登记发布时把 note_id 存到 prediction header，retro 启动时读这个字段。
...
| 笔记列表为空 | 创作者中心改了 galaxy 接口路径 | 看 `.nexttake-cache/xhs-explore-debug/creator_urls.txt`，更新 `GALAXY_*_KEYS` |
...
- `.nexttake-cache/xhs-explore-debug/` 也不应提交（含调试截图 / 接口 URL / 原始 JSON）
