...
- **BLIND_SCORING = on**（默认）/ off —— 是否走 [cheat-score-blind](../cheat-score-blind/WORKFLOW.md) sub-agent。off 等价于 `--skip-blind` flag，标 `last_prediction_self_scored: true` 给 cheat-status 警告
...
详见 [cheat-score-blind/WORKFLOW.md](../cheat-score-blind/WORKFLOW.md) 的"主 Claude 调用契约"段。**Task prompt 必须精简**：
...
3. **Refusal check**：`refusal != null` → 按 [cheat-score-blind/WORKFLOW.md](../cheat-score-blind/WORKFLOW.md) Phase 2 的处理表对应路径
...
- hook 依赖：`hooks/prediction-immutability.sh` 必须已安装在用户 `.claude/settings.json`，否则 immutability 仅靠 WORKFLOW.md 自律——`cheat-status` 会持续提示
