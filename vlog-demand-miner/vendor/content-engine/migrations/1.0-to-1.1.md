...
| 字段 | 默认值 | 写入者 |
|---|---|---|
| `typical_duration_seconds` | 240 | initialize Q1.5 派生 |
| `target_publish_cadence_days` | null（灵活节奏） | initialize Q1.6 派生 |
| `rubric_form_mismatch` | false | initialize（content_form ≠ opinion-video 时 true） |
| `benchmark_status` | "none" | initialize Phase 2.5 |
| `benchmark_name` | null | learn-from |
| `benchmark_sample_count` | 0 | learn-from |
| `baseline_plays` | null | initialize / retro |
| `enabled_perf_adapters` | `[]` | initialize |
| `last_published_file` | null | publish |
| `last_retro_at` | null | retro |
| `pending_retros` | `[]` | publish / retro |
| `shoots` | `[]` | shoot / publish |
...
不想跑 `/migrate` 的用户可以手改 `.nexttake-state.json`：
...
> 手改后建议跑 `/status` 验证 state 文件能被所有 skill 正常读。
