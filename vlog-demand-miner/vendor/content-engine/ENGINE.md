---
name: nexttake-content-engine
description: 给所有想把"感觉"变成可校准预测的内容创作者。**方法论通用**——打分 → 盲预测 → T+3d 复盘 → 进化 rubric 的循环适用任何能被量化（播放 / 阅读 / 收听 / 点击）的内容。**rubric 是循环的内容，不是循环本身**——当前内置一份观点视频 rubric（参考博主 25+ 视频拟合），其他形态可借这套起步并 bump 调权重。**强烈建议导入对标账号**作为初始信号源（/learn-from）。触发词："初始化"/"打分这篇"/"启动预测"/"已发布"/"复盘"/"升级 rubric"/"推荐选题"/"抓热点"/"状态"/"找对标"/"learn from"。**首次使用必须先跑 /initialize。**
...
本文件是**总协议 + 路由器**。具体每个阶段的工作流在 `skills/*/WORKFLOW.md` 各子目录里。

## Codex compatibility

Codex 没有 Claude Code 的 slash-command harness。安装到 Codex 后，按自然语言触发同一套路由即可：

- `初始化 NextTake Content Engine` → 读取并执行 `skills/initialize/WORKFLOW.md`
- `打分这篇 scripts/foo.md` → 读取并执行 `skills/score/WORKFLOW.md`
- `启动预测 scripts/foo.md` → 读取并执行 `skills/predict/WORKFLOW.md`
- `拍了 ...` / `已发布 ...` / `复盘 ...` / `升级 rubric` / `状态` → 分别读取对应的中性工作流目录

执行时遵循本文件的三条原则和路由表；不要依赖 slash command 是否存在。Claude Code 专用 hook（`.claude/settings.json`）仍只在 Claude Code 里自动触发；Codex 中需要用户主动说 `状态` 查看 buffer、待复盘和候选池。
...
| 用户说 | 调用 | 前置条件 |
|---|---|---|
| "初始化" / "init" / "首次使用" | `/initialize` | 无（这是入口） |
| "找对标" / "学这个账号" / "拆这几个对标视频" / "learn from" / "导入对标账号" | `/learn-from` | 已 init；cold-start 强烈建议；后续可随时 --append / --replace |
| "找选题" / "我不知道拍什么" / "seed" / "找前 5 个选题" | `/ideate` | 已 init（cold-start 用户专用一次性种子动作） |
| "打分这篇 [path]" / "score this [path]" | `/score` | rubric_notes.md 存在 |
| "启动预测" / "start prediction" / "给这稿子打分并预测" | `/predict` | 已 init + 有最终稿 |
| "拍了 X" / "shot it" / "录完了" | `/shoot` | 对应预测已写（buffer +1） |
| "已发布" / "I shipped it" / "发布链接是 X" | `/publish` | 对应预测文件存在（buffer -1） |
| "复盘" / "retro this" / "T+3d 数据来了" | `/retro` | 对应预测文件存在 + 已过 RETRO_WINDOW_DAYS |
| "构造受众画像" / "更新 persona" / "我的观众是谁" / "build persona" | `/persona` | 已 init；有复盘评论数据（或 benchmark seed） |
| "升级 rubric" / "bump rubric" / "更新公式" | `/calibrate` | 校准池 ≥ MIN_SAMPLES_FOR_BUMP |
| "推荐选题" / "next topic" | `/recommend` | candidates.md 存在且非空 |
| "抓热点" / "fetch trends" / "今天有什么可做的" | `/trends` | trend-sources adapter 已配置（日常补充候选池） |
| "状态" / "status" / "看板" | `/status` | 任意时刻可调 |
| "迁移" / "升级 state" / "schema 版本不对" / "migrate" | `/migrate` | 已 init；用户 git pull 拉了新版后；SessionStart hook 提示 schema mismatch 后 |
...
1. 检查用户当前目录是否有 `.nexttake-state.json` → 没有 → 强制路由到 `/initialize`
2. 检查 `predictions/` 下有几个文件含完整 `## 复盘` 段填了真实数据 → 决定 `mode: cold-start | calibration`
3. 把判定结果写回 `.nexttake-state.json` 后再路由到目标 skill
...
skill 期望用户的项目布局如下。`/initialize` 会创建缺失项；**绝不在没确认的情况下覆盖**。

```
<user-content-project>/
├── .gitignore                         # initialize 创建；挡住 .auth*/.nexttake-secrets.json 等凭证
├── rubric_notes.md                    # 评分规则的真实来源
├── WORKFLOW.md                        # 5 阶段流程文档（initialize 创建）
├── STATUS.md                          # 看板（status 维护）
├── .nexttake-state.json                  # 状态文件，子 skill 共享上下文
├── .nexttake-cache/                      # 不入版本控制
│   ├── usage.jsonl                    # 钩子被动记录的使用日志
│   └── trends-history.jsonl           # trends 的去重缓存
├── .claude/
│   └── settings.json                  # 含 prediction-immutability hook
├── benchmark.md                       # 对标账号信息（learn-from 维护）
├── audience.md                        # 受众画像（persona 派生；blind 硬禁读）
├── scripts/                           # 拍前的所有草稿（ideate 写或用户写）
│   └── YYYY-MM-DD_<id>_<short>.md
├── predictions/                       # immutable 预测日志（hook 保护）
│   └── YYYY-MM-DD_<id>_<short>.md     # 与 scripts/ 同 id
├── videos/                            # 拍后才建（shoot 创建）
│   └── YYYY-MM-DD_<id>_<short>/
│       ├── script.md                  # 用户提供的最终拍摄稿（shoot 时询问"和 scripts/ 一致吗"）
│       └── report.md                  # T+3d 抓的数据 + 评论（retro 写）
├── samples/                           # 对标账号视频 / 转录（learn-from 创建）
...
```
content-engine/
├── ENGINE.md                          # 本文件（总协议 + 路由）
├── skills/                            # 子 skill 集
│   ├── initialize/WORKFLOW.md            # ✅ 入口：onboarding 与脚手架
│   ├── learn-from/WORKFLOW.md      # ✅ 对标账号导入（拆 pattern + 派生 base rubric 信号）
│   ├── ideate/WORKFLOW.md            # ✅ Cold-start 选题启动器（brainstorm + 可选 draft）
│   ├── score/WORKFLOW.md           # ✅ 单稿打分（不写文件）
│   ├── predict/WORKFLOW.md         # ✅ 盲预测 + immutable 日志
│   ├── shoot/WORKFLOW.md           # ✅ 登记拍摄（buffer +1）
│   ├── publish/WORKFLOW.md         # ✅ 发布元数据登记（buffer -1）
│   ├── retro/WORKFLOW.md           # ✅ 数据回收 + 复盘
│   ├── persona/WORKFLOW.md         # ✅ 受众画像派生（从复盘评论聚类）
│   ├── calibrate/WORKFLOW.md            # ✅ rubric 升级（含跨模型审）
│   ├── recommend/WORKFLOW.md       # ✅ 候选池排序推荐（按 buffer 颜色 + 1 稳 + 1 实验）
│   ├── trends/WORKFLOW.md          # ✅ 热点抓取（日常补充候选池，多 adapter）
│   ├── status/WORKFLOW.md          # ✅ 状态看板（含 buffer 警戒）
│   ├── migrate/WORKFLOW.md         # ✅ schema 升级（老用户 git pull 后用）
│   └── score-blind/WORKFLOW.md     # ✅ Channel B 隔离打分 sub-agent（仅 Task tool 调用）
...
│   ├── state-management.md            # ✅ .nexttake-state.json 读写约定
...
│   ├── long-essay-zero.md             # ✅ 长文 cold-start
│   ├── short-text-zero.md             # ✅ 短文本 cold-start
│   ├── podcast-zero.md                # ✅ 播客 cold-start
│   ├── tutorial-builder-zero.md       # ✅ 教程 cold-start
│   └── other-zero.md                  # ✅ 其他形态 cold-start
...
│   ├── trend-sources/                 # 热点抓取源
│   └── script-extraction/             # 视频/音频转 script（含 whisper for learn-from）
└── examples/
...
- 宣传性 hook 不属于运行协议——**不要写进** `rubric_notes.md` 或预测日志
...
- 修改路由 → 改本文件的"路由表"段
- 子工作流内部细节 → 直接改对应 `skills/<workflow>/WORKFLOW.md`

项目级公共契约见 `../../SKILL.md`；NextTake 路由边界见 `../../references/internal-content-engine.md`。
