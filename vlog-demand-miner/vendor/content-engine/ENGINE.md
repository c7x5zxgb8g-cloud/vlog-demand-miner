...
本文件是**总协议 + 路由器**。具体每个阶段的工作流在 `skills/cheat-*/WORKFLOW.md` 各子 skill 里。

## Codex compatibility

Codex 没有 Claude Code 的 slash-command harness。安装到 Codex 后，按自然语言触发同一套路由即可：

- `初始化 cheat-on-content` → 读取并执行 `skills/cheat-init/WORKFLOW.md`
- `打分这篇 scripts/foo.md` → 读取并执行 `skills/cheat-score/WORKFLOW.md`
- `启动预测 scripts/foo.md` → 读取并执行 `skills/cheat-predict/WORKFLOW.md`
- `拍了 ...` / `已发布 ...` / `复盘 ...` / `升级 rubric` / `状态` → 分别读取对应 `skills/cheat-*/WORKFLOW.md`
...
├── README.md                          # 营销门面
├── skills/                            # 子 skill 集
│   ├── cheat-init/WORKFLOW.md            # ✅ 入口：onboarding 与脚手架
│   ├── cheat-learn-from/WORKFLOW.md      # ✅ 对标账号导入（拆 pattern + 派生 base rubric 信号）
│   ├── cheat-seed/WORKFLOW.md            # ✅ Cold-start 选题启动器（brainstorm + 可选 draft）
│   ├── cheat-score/WORKFLOW.md           # ✅ 单稿打分（不写文件）
│   ├── cheat-predict/WORKFLOW.md         # ✅ 盲预测 + immutable 日志
│   ├── cheat-shoot/WORKFLOW.md           # ✅ 登记拍摄（buffer +1）
│   ├── cheat-publish/WORKFLOW.md         # ✅ 发布元数据登记（buffer -1）
│   ├── cheat-retro/WORKFLOW.md           # ✅ 数据回收 + 复盘
│   ├── cheat-persona/WORKFLOW.md         # ✅ 受众画像派生（从复盘评论聚类）
│   ├── cheat-bump/WORKFLOW.md            # ✅ rubric 升级（含跨模型审）
│   ├── cheat-recommend/WORKFLOW.md       # ✅ 候选池排序推荐（按 buffer 颜色 + 1 稳 + 1 实验）
│   ├── cheat-trends/WORKFLOW.md          # ✅ 热点抓取（日常补充候选池，多 adapter）
│   ├── cheat-status/WORKFLOW.md          # ✅ 状态看板（含 buffer 警戒）
│   ├── cheat-migrate/WORKFLOW.md         # ✅ schema 升级（老用户 git pull 后用）
│   └── cheat-score-blind/WORKFLOW.md     # ✅ Channel B 隔离打分 sub-agent（仅 Task tool 调用）
...
- 修改路由 → 改本文件的"路由表"段
- 子 skill 内部细节 → 直接改对应 `skills/cheat-*/WORKFLOW.md`
