...
**Known limitations**（写进 cheat-persona/WORKFLOW.md）：评论 ≠ 全部受众（偏向会评论的活跃少数）；评论可被水军污染；persona 滞后于真实受众变化；不替用户做"想要的受众 vs 实际受众"的战略决策。
...
**改动**：引入 [skills/cheat-score-blind](skills/cheat-score-blind/WORKFLOW.md) 作为 **channel B** 隔离打分 sub-agent。三 channel 模型：
...
### Known limitations（写进 cheat-score-blind/WORKFLOW.md）
