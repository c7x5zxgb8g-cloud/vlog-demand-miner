...
**Script Hash**: `sha256:<12 位>` (predict 时 hash script 内容；登记已拍摄 时再 hash，不一致则复盘段加 integrity warning)
...
> 如用户从零写没用 生成内容文案，写"用户原创稿，无 Claude 草稿对照"。
...
> ⚠️ **以下段落由 `复盘本期内容` 在 T+`RETRO_WINDOW_DAYS` 天后追加**。
> hook 允许追加本段；不允许改预测段任何字符。

（待填——T+RETRO_WINDOW_DAYS 天后跑 `复盘本期内容 <对应 video folder>`）
