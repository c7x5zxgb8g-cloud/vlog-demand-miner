...
> **本文件由 `抓取热点` 写入热点抓取结果，由 `推荐下一条` 读取并排序。
> 也可手动编辑——把候选标题贴成 H3 entry 即可。**
>
> 候选项 Schema 由 NextTake 统一维护。

---

## 使用说明（首次见到本文件请读）

每个候选项是一个 H3 entry（`### [tier] 标题`），下面带 metadata bullets。最简版本只需要 title 一行——`推荐下一条` 会自动调 `给本期文案打分` 给未打分的 entry 粗打分。

### 字段含义速查

- **id**：12 位 hash，用于跨文件去重。手加 entry 留空，`抓取热点` 自动算
...
剩下的字段会被 `推荐下一条` 在下次调用时补全（自动调 `给本期文案打分`）。
...
> done tier 的 entry **不出现在 `推荐下一条` 的输出**——已发过的不再推。
...
- **定期清理 skip**：超过 6 个月的 skip 会从 NextTake 本地缓存自动剔除（“抓取热点”重复抓到时会再出现）
- **risky 标签认真用**：`推荐下一条` 会高亮 risky 项让你二次确认；不要把所有"有点争议"都标 risky，会麻木
