...
> **本文件是 NextTake 评分规则进化的载体**。每次复盘实际播放数据 vs 预测分数后，把判断依据和规律显式写在这里，下次打分前 `给本期文案打分` `启动发布前预测` 会先读这个文件再动手。
>
> **核心原则**：规律必须可追溯到具体样本。不写"情感共鸣很重要"这种空话，要写"XX 这篇 ER=5 得到验证 / 推翻，因为评论区 top 3 都是 YY 模式"。
>
> 观察生命周期和升级验证规则由 NextTake 统一维护。
...
> Cold-start 用户应该等权起步，校准 5 篇之后再调整权重。
...
> ```
>
> 删除规则：被吸收为维度或被推翻的观察应删除，git history 才是档案。
...
>
> 本工具按校准阶段切换三种 bucket 方案。当前生效方案由 NextTake 项目状态决定。
...
`查看创作状态` 在 N=10 时主动建议切换。这种方案永远自洽——不管账号多大，"top 5%"语义稳定。
