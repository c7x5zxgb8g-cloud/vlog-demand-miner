...
> Revised after confirming that the complete `NextTake Content Engine` capability set must be preserved in this repository.
...
- complete pinned `NextTake Content Engine` vendoring with MIT license, provenance and SHA-256 manifest;
...
- native lifecycle file attachment without reimplementing internal workflow rules;
...
在 7 天、50 个开发小时内，把现有 Vlog Demand Miner 改造成可参加“个人创作者内容创作与发布系统”赛道的 Agent Skill：从真实同赛道视频和评论中发现值得创作的主题，生成可拍摄内容，复用完整 `NextTake Content Engine` 创作实验闭环，登记人工发布结果，并把播放、互动和评论转成下一条内容建议。
...
1. **能力资产完整保留**：把固定版本的 `NextTake Content Engine` 全量复制进仓库，不丢弃子 Skill、协议、模板、迁移、Hooks、Rubric、工具和 Adapter；
...
- 完整 vendor 固定版本 `NextTake Content Engine`，保留 MIT `LICENSE` 和上游来源说明；
- `NextTake Content Engine` 已有的同类能力必须直接调用或编排，禁止在 NextTake 中重新实现第二套 `seed/score/predict/shoot/publish/retro/persona/recommend`；
- 新增代码必须能说明它填补的是 VDM 与 `NextTake Content Engine` 之间的真实能力缺口，而不是仅因接口形式不同就重造轮子；
- vendor 清单与固定上游版本一致，排除 `.git`、`__pycache__`、`.pyc`、Cookie、登录态、虚拟环境和本机调试文件；
- 原有 15 个内部工作流、Hooks、Migrations、Shared References、Starter Rubrics、Templates、Tools 和 Adapters 均被保留；
...
NextTake 不是重新实现一个缩水版 `NextTake Content Engine`，而是把两个已有优势组合起来。默认决策是 **reuse first**：只要上游已经有同样的业务语义，就直接复用；只有上游确实不存在的能力才允许新增。
...
Vlog Demand Miner
  真实同赛道视频、评论、Evidence、需求 Cluster
            ...
            v
NextTake Bridge
  Evidence -> content candidate/source pack
            |
            v
Vendored NextTake Content Engine
...
- **VDM** 回答“市场在问什么、证据在哪里”；
- **`ideate`** 在 VDM Evidence 约束下回答“基于这些证据，这条具体怎么拍”；
- **`NextTake Content Engine`** 回答“发布前怎么下注、发布后怎么对账、长期怎么进化”；
...
1. `NextTake Content Engine` 是否已有相同业务语义；
...
| --- | --- | --- |
| Topic discussion and draft generation | `ideate` | 注入 VDM source pack，不另写生成器 |
| Draft scoring | `score` | 直接路由，不另建评分 schema |
| Blind prediction | `predict` | 直接路由并保持 immutable |
| Shoot registration | `shoot` | 直接路由 |
| Manual publication registration | `publish` | 直接路由，不实现平台上传 |
| Performance and comment retro | `retro` | 直接路由；仅做输入清洗和确定性指标校验 |
| Audience persona | `persona` | 直接路由 |
| Next-topic recommendation | `recommend` | 直接路由 |
| Trends, rubric evolution, status, migration | Existing internal workflows | 完整保留，按本周优先级暴露 |
...
源码层面仍完整保留所有 `NextTake Content Engine` 能力；MVP 边界只约束本周接线、测试和 UI，不约束仓库中保留的功能。
...
```text
vlog-demand-miner/vendor/content-engine/
```
...
- `vendor/content-engine/UPSTREAM.md`：记录仓库、固定 commit、vendor 日期、许可证和本地适配策略；
- `vendor/content-engine/MANIFEST.sha256`：对清理后的 vendored 文件生成哈希清单；
- `references/content-engine-integration.md`：记录 NextTake 调用边界和已验证能力矩阵。
...
- 虚拟环境、下载模型、运行日志和本机缓存；
- 任何用户内容项目中的私有 `predictions/`、`videos/` 或 `.nexttake-state.json`。
...
2. **Creator Project 域**：由 `NextTake Content Engine` 管理的 `scripts/`、`predictions/`、`videos/`、`candidates.md`、`audience.md`、`rubric_notes.md` 和 `.nexttake-state.json`。
...
| --- | --- | --- |
| `scripts/vdm.py` | 参数解析和薄命令路由 | 不重写内部内容生命周期 |
| `scripts/content.py` | Opportunity、Evidence source pack 和确定性指标的纯验证 | 不生成脚本、不评分、不复盘 |
| `scripts/creator_flow.py` | VDM Artifact 与原生 creator project 文件之间的薄桥接 | 不复制 `ideate` 或其他生命周期逻辑 |
| `scripts/creator_reports.py` | 安全生成 Creator Studio HTML | 不修改业务状态 |
| `vendor/content-engine/` | 原生创作实验协议和全部能力 | 不依赖 NextTake 才能保持完整 |
| Root `SKILL.md` | 将自然语言请求路由到 VDM、生成桥或 vendored Skill | 不复制子 Skill 规则到另一套 prompt |

### Self-Contained Runtime

- 新增 `NEXTTAKE_CONTENT_ENGINE_ROOT`，默认指向仓库内 vendored 根目录；
- `NEXTTAKE_DOUYIN_ADAPTER_DIR` 仍可显式覆盖；
...
NextTake 把 Opportunity 转成 `ideate` 可读取的 source pack。原生 `ideate` 读取 Opportunity、creator profile、`audience.md` 和允许读取的写作模式，生成：
...
生成稿由 `ideate` 写入 creator project 的 `scripts/`，随后直接调用原生 `score` 与 `predict`。NextTake 不创建自有 draft、score 或 prediction 协议。

### 3. Publish

- 使用原生 `shoot` 登记已拍摄；
- 使用原生 `publish` 登记人工发布平台、时间和链接；
- 不调用平台上传 API；
- 演示 fixture 明确标记为 Demo publication。

### 4. Learn

- 使用原生 `retro` 写入原始表现数据与评论；
- NextTake 确定性重算互动率并提供给 Studio；
- 使用原生 `persona` 从评论派生受众画像；
- 使用原生 `recommend` 从候选池产生下一条建议；
- 长期样本达到门槛后，完整保留的 `calibrate` 可升级 Rubric，但不作为本周演示硬条件。
...
- 仓库内存在可追溯、无私有状态、可哈希验证的完整 `NextTake Content Engine` 源码。
...
- 让原生 `ideate` 基于 source pack 生成带 Evidence/Creator Original 标签的脚本；
...
- 不允许通过新增独立脚本生成器来绕过 `ideate` 的兼容问题。

### Hours 18-31: Native Creator Lifecycle Integration

Actions:

- Root `SKILL.md` 路由到 vendored `initialize/seed/score/predict/shoot/publish/retro/persona/recommend`；
...
- `vlog-demand-miner/vendor/content-engine/**`；
- `vlog-demand-miner/vendor/content-engine/UPSTREAM.md`；
- `vlog-demand-miner/vendor/content-engine/MANIFEST.sha256`；
...
- `vlog-demand-miner/references/content-engine-integration.md`；
...
- VDM Cluster -> source pack -> native `ideate` draft；
...
4. `1:20-1:40`：原生 `score/predict` 完成发布前打分和盲预测；
...
- 从“借鉴 `NextTake Content Engine`”改为“完整 vendor 并原生复用”；
...
补充约束：`ideate` 同样作为脚本生成主入口，NextTake 不再规划独立内容生成器。VDM 只向它提供更高质量、更可追溯的 source pack。
...
3. 完整 vendor 固定版本 `NextTake Content Engine`，不删除已有功能；
