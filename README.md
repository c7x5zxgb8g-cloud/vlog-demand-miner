...
```text
init
  -> creator-discover（按赛道自动检索）或 creator-add（用户提供账号）
  -> sync
  -> sample
  -> acquire
...
只提供赛道名称即可。NextTake 默认生成不超过 3 个搜索词，每个平台只搜索第 1 页，去重排序后自动登记前 `1-5` 个候选账号：
...
B站账号搜索直接复用固定版本 CLI。抖音 Sidecar 当前负责已知账号采集和分享链接解析；抖音自动搜索使用 Browser Provider 的公开可见搜索页，需要先完成一次人工浏览器登录。搜索候选表示内容相关性，不证明账号质量、赛道需求或未来播放表现。
...
当 Sidecar 的 `status` 为 `ok` 时，已知抖音账号的解析和采集直接使用 `--douyin-provider sidecar`。当前默认地址是 `http://127.0.0.1:18080`，NextTake 只检查本地 OpenAPI 能力，不读取容器内凭证。只有需要“按赛道自动搜索抖音账号”时，才需要额外安装 Browser Provider 并人工登录一次。
...
| `init` | 初始化 VDM 研究项目 |
| `creator-discover` | 根据赛道自动检索并登记对标账号 |
| `creator-add` | 登记同赛道博主 |
| `sync` | 同步作品库存 |
| `sample` | 选择待研究作品 |
