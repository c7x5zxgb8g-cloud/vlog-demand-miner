...
- 抖音默认导入已知对标账号，关键词发现仅在显式开启实验开关后运行；
- 抖音 Browser 采集复用单个持久会话，同一视频的公开信息和评论只导航一次；
...
```text
init
  -> creator-discover（B站自动；抖音仅显式实验）或 creator-add（抖音默认）
  -> sync
  -> sample
  -> acquire
...
B站只提供赛道名称即可：NextTake 默认生成不超过 3 个搜索词，只搜索第 1 页，去重排序后自动登记前 `1-5` 个候选账号：

```bash
python3 "$VDM_CLI" --project "$RESEARCH_DIR" creator-discover \
  --track "首次租房" \
  --platform bilibili \
  --limit 3
```

也可以由 AI 根据赛道补充关键词，但最多使用 3 个：
...
B站账号搜索直接复用固定版本 CLI。抖音默认走下面的“用户提供对标账号”模式；未显式开启时，`creator-discover --platform douyin` 返回 `manual_input_required`，不会启动浏览器搜索。

抖音按赛道搜索只保留为实验能力，必须完成一次人工浏览器登录并显式开启：

```bash
python3 "$VDM_CLI" --project "$RESEARCH_DIR" --douyin-provider browser creator-discover \
  --track "首次租房" \
  --platform douyin \
  --limit 3 \
  --enable-experimental-douyin-discovery
```

搜索候选表示内容相关性，不证明账号质量、赛道需求或未来播放表现；出现登录、验证、风控或异常空结果时会保留检查点并停止，不自动重试。

### 模式二：用户提供对标账号

支持账号 ID、B站主页 URL、抖音主页 URL和抖音分享短链：
...
python3 "$VDM_CLI" --project "$RESEARCH_DIR" creator-add \
  --name "用户提供的抖音账号" \
  --platform douyin \
  --account-url "https://v.douyin.com/<分享短链>/"
...
NextTake 不承诺消除平台风控，也不会尝试绕过验证。Browser 路径把抖音公开页面访问收敛为以下受控流程：

1. 默认使用 `creator-add` 导入账号 ID、主页 URL 或分享短链，不主动进行关键词搜索；
2. 只有明确传入 `--enable-experimental-douyin-discovery` 时，才运行实验性的关键词发现；
3. 登录由使用者在普通浏览器窗口中手动完成，登录态保存在仓库外的持久 Profile，NextTake 不读取或复制 Cookie；
4. 同一个 `sync` 或 `acquire` 任务只创建一个持久 Browser 会话，并串行执行操作；
5. 同一视频的公开信息与评论采集合并为一次视频页导航，避免 `fetch_post` 和 `fetch_comments` 重复打开页面；
6. Provider 操作之间默认等待 `6-12` 秒，项目级门控也会阻止多个 CLI 进程意外并发突发；
7. 遇到 `blocked_auth`、`blocked_verification`、`risk_control`、`schema_drift` 或 `anomalous_empty_result` 时立即停止剩余计划并保留检查点，不自动重试。

这种策略降低了搜索、重复导航、并发和盲目重试带来的额外风险，但平台仍可能基于账号、网络、设备或访问行为触发验证。
...
当 Sidecar 的 `status` 为 `ok` 时，已知抖音账号的解析和采集直接使用 `--douyin-provider sidecar`。当前默认地址是 `http://127.0.0.1:18080`，NextTake 只检查本地 OpenAPI 能力，不读取容器内凭证。只有需要显式实验性的“按赛道搜索抖音账号”时，才需要额外安装 Browser Provider 并人工登录一次。
...
抖音关键词 Browser 搜索默认关闭。使用 `creator-add` 提供账号 ID、主页 URL 或分享 URL；只有明确接受实验性平台风险时，才为 `creator-discover` 增加 `--enable-experimental-douyin-discovery`。

### 抖音 Browser 停止状态

以下状态都表示采集已经停止并保留检查点，不要连续重试：

| Status | Meaning | Recovery |
| --- | --- | --- |
| `blocked_auth` | 登录失效或进入登录页 | 重新执行 `douyin-login` 完成人工登录，再运行 `doctor` |
| `blocked_verification` | 页面要求验证码或安全验证 | 在可见浏览器中人工完成平台检查，确认页面恢复正常后再继续 |
| `risk_control` | 平台提示访问频繁或风险受限 | 停止访问，等待平台恢复；不要缩短间隔或自动重试 |
| `schema_drift` | 当前 Browser Adapter 与页面/调用协议不兼容 | 安装或更新兼容 Adapter，通过 `doctor` 后再继续 |
| `anomalous_empty_result` | 页面并非明确无评论，但采集结果异常为空 | 人工检查可见页面；不要把异常空结果当作“没有需求” |

人工检查和环境修复完成后，可从未完成任务继续：

```bash
python3 "$VDM_CLI" --project "$RESEARCH_DIR" --douyin-provider browser \
  douyin-login --wait-seconds 300
python3 "$VDM_CLI" --project "$RESEARCH_DIR" --douyin-provider browser doctor
python3 "$VDM_CLI" --project "$RESEARCH_DIR" --douyin-provider browser resume
```
...
| `init` | 初始化 VDM 研究项目 |
| `creator-discover` | B站按赛道自动检索；抖音仅显式实验 |
| `creator-add` | 登记同赛道博主 |
| `sync` | 同步作品库存 |
| `sample` | 选择待研究作品 |
...
| 抖音 Browser Adapter | 已接入；已知 URL 低频采集、任务级单会话、同视频单次导航，关键词发现仅显式实验，真实使用需要人工登录 |
| B站、XHS、LinkedIn 上游 Adapter | 源码已保留，并非全部完成真实账号验证 |
| 自动上传或定时发布 | 不支持，发布由创作者手动完成 |
| 云端 SaaS、多用户和团队协作 | 不在当前范围 |

不要把“源码已保留”理解为“所有外部 Adapter 已完成真实平台端到端验证”。2026-07-22 的本次更新已通过代码检查和自动测试验证；由于本机 Browser Runtime 不可用，本次没有访问真实抖音页面完成端到端验证。
